...
#     query = "SELECT responses FROM PHQ_9forms WHERE appointment_id = %s"
...
# Generate markdown for responses
def generate_responses_markdown(answered_questions):
    response_values = {
        "Not at all": 0,
        "Several Days": 1,
...
# Fetch captured responses
def fetch_captured_responses(db, appointment_id):
    cursor = db.cursor()
    query = "SELECT responses FROM PHQ_9forms WHERE appointment_id = ?"
    cursor.execute(query, (appointment_id,))
    row = cursor.fetchone()
    cursor.close()
