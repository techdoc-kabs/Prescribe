...
def main():
    ...
    st.sidebar.write('  ')
    with st.sidebar.expander('Multiple students', expanded=True):
        df = fetch_filtered_appointments(db)
        latest_term = df["term"].dropna().sort_values().iloc[-1] if not df["term"].isna().all() else None
        latest_screen_type = df["screen_type"].dropna().sort_values().iloc[-1] if not df["screen_type"].isna().all() else None
        term_filter = st.selectbox("Select Term", df["term"].unique(), index=list(df["term"].unique()).index(latest_term))
        screen_type_filter = st.selectbox("Select Screening Type", df["screen_type"].unique(), index=list(df["screen_type"].unique()).index(latest_screen_type))
        df = df[(df["term"] == term_filter) & (df["screen_type"] == screen_type_filter)]
        if df.empty:
            st.warning("No matching students found.")
            st.stop()
    tools_to_add_per_student = {}
    with c2.form('form'):
        st.write(f'No.of Students: {len(df)}')
        selected_tools = st.multiselect("Select tools", ['PHQ-9', 'GAD-7', 'SRQ', 'BDI', 'CAGE', 'SAD PERSONS SCALE'])
        ...
        if selected_tools:
            for _, record in df.iterrows():
                ...
                pre_screen_exists = any(
                    (row['student_id'] == student_id and row['appointment_id'] == appointment_id and row['screen_type'] == 'PRE-SCREEN')
                ...
                for tool in selected_tools:
                    if tool in tools_in_appointment:
                        blocked_tools.append(f"{tool} (Already exists in appointment {appointment_id})")
                    elif tool in tools_for_student and tools_for_student[tool] == "Pending":
                        blocked_tools.append(f"{tool} is still pending from a previous appointment for {name}")
                    elif screen_type_filter == 'POST-SCREEN' and not pre_screen_exists:
                        blocked_tools.append(f"{tool} cannot be added to POST-SCREEN without a PRE-SCREEN for {name}")
                    else:
                        tools_to_add.append(tool)

                if blocked_tools:
                    st.warning(f"Cannot add the following tools for {name}: {', '.join(blocked_tools)}")
                if tools_to_add:
                    tools_to_add_per_student[(student_id, appointment_id)] = tools_to_add

        add = st.form_submit_button(':green[Add]')
        if add and tools_to_add_per_student:
            for (student_id, appointment_id), tools in tools_to_add_per_student.items():
                insert_requested_tools_students(db, student_id, appointment_id, term_filter, screen_type_filter, tools)
            st.success("Tools successfully added for all selected students!")
        ...
        if add and tools_to_add_per_student:
            for (student_id, appointment_id), tools in tools_to_add_per_student.items():
                ...
                send_notifications(student_name, ', '.join(tools), datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            st.success("Tools successfully added for all selected students!")

    with c1.expander("REMOVE TOOL", expanded=True):
        all_tools = set()
        for _, record in df.iterrows():
            student_id = record['student_id']
            appointment_id = record['appointment_id']
            tools_in_db_df = fetch_requested_tools_df(db, student_id, appointment_id)
            all_tools.update(tools_in_db_df['Tool Name'].tolist())
        if all_tools:
            tool_to_remove = st.selectbox("Select Tool", list(all_tools))
            remove_all = st.button(f":red[Delete]")
            ...
            if remove_all:
                ...
                st.success(f"Removed {tool_to_remove}!")

    if c1.checkbox('View added tools'):
        tools_summary = []
        for _, record in df.iterrows():
            student_id = record['student_id']
            appointment_id = record['appointment_id']
            name = record['name']
            tools_in_db_df = fetch_requested_tools_df(db, student_id, appointment_id)
            for tool, status in zip(tools_in_db_df['Tool Name'], tools_in_db_df['Status']):
                tools_summary.append({
                    "Student ID": student_id,
                    "Student Name": name,  
                    "Appointment ID": appointment_id,
                    "Term": term_filter,  
                    "Screen Type": screen_type_filter,  
                    "Tool": tool,
                    "Status": status
         
                })
        if tools_summary:
            summary_df = pd.DataFrame(tools_summary)
            summary_df.index = summary_df.index + 1
            st.write(summary_df)
        else:
            st.warning('No existing tools for filtered appointmnets')
            st.write(tools_summary)
    db.close()
