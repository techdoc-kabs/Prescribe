...
def get_student_info(db):
    query = """
    SELECT student_id, name, student_class, stream, gender 
    FROM student_users
    """
...
def generate_tool_impact(db, table_name, score_col, status_col, tool_label):
    query = f"""
        ...
        FROM requested_tools_students rts
        JOIN {table_name} f ON rts.appointment_id = f.appointment_id
        ORDER BY rts.student_id, f.assessment_date DESC
    """
    df = pd.read_sql_query(query, db)
    if df.empty:
        return pd.DataFrame()

    pre = df[df["screen_type"] == "PRE-SCREEN"].groupby("student_id").first().reset_index()[[
        "student_id", "score", "status"]]
    post = df[df["screen_type"] == "POST-SCREEN"].groupby("student_id").first().reset_index()[[
        "student_id", "score", "status"]]

    pre["score"] = pd.to_numeric(pre["score"], errors='coerce')
    post["score"] = pd.to_numeric(post["score"], errors='coerce')

    merged = pd.merge(pre, post, on="student_id", suffixes=("_pre", "_post"))
    merged["score_change"] = merged["score_pre"] - merged["score_post"]

    def categorize(row):
        if row["score_change"] > 0:
            return "Improved"
        elif row["score_change"] < 0:
            return "Deteriorated"
        else:
            return "No Change"
    ...
    return merged[[
        "student_id", "tool", "score_pre", "status_pre",
        "score_post", "status_post", "score_change", "category"
    ]]
...
def style_dataframe(df):
    ...
    return df.style.applymap(highlight_category, subset=["Category"]).set_table_styles([
        {"selector": "th", "props": [("background-color", "#e0f0ff")]}
    ])
...
def apply_dynamic_filters(df):
    ...
    if impact_menu == "All Results":
        ...
        with st.sidebar.expander(' ###FILTER OPTIONS', expanded=True):
            search_by = st.selectbox("Search by", ["Student Name", "Student ID"])
            if search_by == "Student Name" and "Name" in df.columns:
            ...
            elif search_by == "Student ID" and "Student ID" in df.columns:
                id_query = st.text_input("Enter student ID")
                if id_query:
                    filtered_df = filtered_df[filtered_df["Student ID"].astype(str).str.contains(id_query, case=False, na=False)]
...
def generate_impact_pdf(df, include_table, include_summary, include_chart, chart_type):
    ...
    if include_table:
        ...
        data = [["Name", "Student ID", "Class", "Stream", "Gender", "Tool", "Pre", "Post", "Change", "Category"]]
        for _, row in df.iterrows():
            data.append([
                row["Name"], row["Student ID"], row["Class"], row["Stream"], row["Gender"],
...
def main():
    ...
    if db:
        ...
        phq = generate_tool_impact(db, "phq_9forms", "phq_score", "depression_status", "Depression (PHQ-9)")
        gad = generate_tool_impact(db, "gad_7forms", "gad_score", "anxiety_status", "Anxiety (GAD-7)")
        suicide = generate_tool_impact(db, "phq_9forms", "suicide_response", "suicide_risk", "Suicide Risk")
        functioning = generate_tool_impact(db, "functioning_responses", "fnx_score", "difficulty_level", "Functioning")
        combined = pd.concat([phq, gad, suicide, functioning], ignore_index=True)
        if not combined.empty:
            combined = pd.merge(combined, student_info, on="student_id", how="left")
            ...
            if selected == "Dataframe":
                ...
                styled_df = style_dataframe(combined[[
                    "Name", "Student ID", "Class", "Stream", "Gender", "Tool",
